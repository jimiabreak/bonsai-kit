...
export default function SubscriptionPage() {
  ...
  const [processingVariantId, setProcessingVariantId] = useState<string | null>(
    null,
  );
  const MAX_RETRIES = 2;
  ...
  useEffect(() => {
    ...
    async function loadProducts() {
      try {
        ...
        setEspressoOptions(
          espresso.length >= 2 ? espresso : getFallbackEspressoOptions(),
        );
      } catch (err) {
        ...
        if (retryAttempt < MAX_RETRIES) {
          ...
          setError(
            "Unable to load latest pricing. Showing standard subscription options.",
          );
        }
      } finally {
        setIsLoading(false);
    ...
    return product.variants.edges
      ...
      .map(({ node: variant }) => {
        ...
        return {
          ...
          usageRate:
            bags === 2
              ? "YOU GO THROUGH 1 BAG A WEEK"
              : "YOU GO THROUGH 0.5 BAG A WEEK",
  ...
  const handleSignUp = async (option: SubscriptionOption) => {
    ...
    if (!option.shopifyVariantId) {
      ...
      alert(
        "This subscription is currently unavailable. Please try another option or contact us for assistance.",
      );
      return;
    }
  ...
  return (
    <div className="bg-cream min-h-screen w-full overflow-x-hidden">
      ...
      <motion.div
        ...
        transition={{
          duration: 0.6,
          delay: !prefersReducedMotion ? 0.25 : 0,
          ease: "easeOut",
        }}
      >
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 md:gap-16 lg:gap-32 items-start">
          <div>
            ...
            <p className="text-brand-blue text-[18px] sm:text-[22px] md:text-[28px] lg:text-[33px] leading-[1.4] md:leading-[44px] tracking-[-0.04em]">
              Select an amount and frequency below. Checkout and enjoy free
              shipping. Get our freshest coffee, straight to your door.
            </p>
          </div>
      ...
      {isLoading ? (
        <motion.div
          ...
          transition={{
            duration: 0.6,
            delay: !prefersReducedMotion ? 0.4 : 0,
            ease: "easeOut",
          }}
        >
      ...
      <div className="max-w-[1512px] mx-auto px-4 md:px-8 mb-24 md:mb-32 grid grid-cols-1 md:grid-cols-2 gap-8 md:gap-12 items-center">
        <motion.div
          ...
          transition={{
            duration: 0.6,
            delay: !prefersReducedMotion ? 0.5 : 0,
            ease: "easeOut",
          }}
        >
          ...
          <div className="text-brand-blue text-[18px] sm:text-[22px] md:text-[28px] lg:text-[33px] leading-[1.4] md:leading-[44px] tracking-[-0.04em] space-y-4 md:space-y-6">
            <p>
              Commonwealth Coffee Roasters has been a cornerstone of quality in
              Birmingham, Michigan, delivering exceptional specialty coffee
              since 2010 at Commonwealth Cafe.
            </p>
            <p>
              We bring ethically sourced, expertly roasted beans right to your
              door. Our passion lies in showcasing the world&apos;s finest
              coffees, sourced from the world&apos;s top tier origins and
              producers, roasted with precision to unlock their fullest
              potential—so that you savor every sip. Subscribe today.
            </p>
          </div>
        </motion.div>
        ...
        <motion.div
          ...
          transition={{
            duration: 0.6,
            delay: !prefersReducedMotion ? 0.6 : 0,
            ease: "easeOut",
          }}
        >
          <video
          ...
          >
            <source
              src="/videos/FullSizeRender_VSCO-transcode.webm"
              type="video/webm"
            />
            <source
              src="/videos/FullSizeRender_VSCO-transcode.mp4"
              type="video/mp4"
            />
          </video>
        </motion.div>
      ...
      <motion.div
        ...
        transition={{
          duration: 0.6,
          delay: !prefersReducedMotion ? 0.7 : 0,
          ease: "easeOut",
        }}
      >
        <Image
      ...
      <motion.div
        ...
        transition={{
          duration: 0.6,
          delay: !prefersReducedMotion ? 0.8 : 0,
          ease: "easeOut",
        }}
      >
          ...
          items={[
            {
              ...
              answer:
                "Yes! You can manage your subscription anytime through your Shopify customer account. Login to update payment methods, change delivery frequency, pause, or cancel your subscription. Click 'Manage Your Subscription' above or visit your customer portal.",
            },
            {
            ...
            },
            {
              ...
              answer:
                "Subscribers always get our freshest coffee. Our beans are roasted and shipped biweekly on Tuesdays from our metro Detroit facility.",
            },
            {
              question: "When can I expect my order?",
              answer:
                "Subscriptions are shipped biweekly on Tuesdays from our metro Detroit facility. You will be emailed a tracking number when your order ships.",
            },
          ]}
        />
      </motion.div>
